...
export async function migrate() {
  ...
  // Unique constraint on (source_url, species_slug) — allows same article across species for general feeds
  ...
  // Track URLs from general feeds that were classified but matched no species,
  // so we don't waste AI calls re-classifying them on every run
  await exec(`
    CREATE TABLE IF NOT EXISTS classified_urls (
      id          SERIAL PRIMARY KEY,
...
/** Check if a URL was already classified (matched or not) from a general feed */
export async function classifiedUrlExists(sourceUrl) {
  const rows = await query(`SELECT id FROM classified_urls WHERE source_url = $1 LIMIT 1`, [sourceUrl])
  return rows.length > 0
}

/** Mark a URL as classified so we don't re-classify it on future runs */
export async function markClassifiedUrl(sourceUrl, feedId) {
  await query(
    `INSERT INTO classified_urls (source_url, feed_id) VALUES ($1, $2) ON CONFLICT (source_url) DO NOTHING`,
    [sourceUrl, feedId]
  )
}

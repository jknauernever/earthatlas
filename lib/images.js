 ...
 */
export async function resolveImage({ rssImage, articleUrl, imageKeywords }) {
  // 1. Already have an image from RSS
  if (rssImage) return { url: rssImage, credit: null }

  // 2. Scrape og:image from the article
  if (articleUrl) {
    const ogImage = await fetchOgImage(articleUrl)
    if (ogImage) return { url: ogImage, credit: null }
  }

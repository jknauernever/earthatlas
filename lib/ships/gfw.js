...
export function mapEntry(entry) {
  ...
  const period = (from, to, where) => {
    try {
      ...
      // Real GFW records can end before they start. Keep the claim, drop the
      // impossible dates (the raw record keeps them); never swap or guess.
      if (f && t && f > t) {
        warnings.push(`${where}: start ${f} is after end ${t}; stored with unknown dates`)

...
/**
 * Latest known value per attribute, for result lists. "Latest" means the
 * greatest period end; registry beats AIS on ties. Each value keeps its
...
async function summarize(q, S, ids) {
  const rows = await q(
    `SELECT DISTINCT ON (va.vessel_id, va.attribute)
      ...
      ORDER BY va.vessel_id, va.attribute, upper(va.period) DESC NULLS LAST,
               (va.evidence_class = 'registry') DESC, va.id`, [ids, SUMMARY_ATTRS])
  const out = {}
  for (const r of rows) {
...
export async function getVessel(q, S, id) {
  ...
  const sources = sourceIds.length
    ...
    : []
  return { vessel, assertions, links, outgoing, sources }
}

...
/**
 * Re-resolve only one source's entities (e.g. 'wikidata'). Safe for
 * attach-only sources: their links are superseded (kept) and re-decided, and
 * no vessel is created or retired, so vessel ids don't change. Evidence untouched.
 */
export async function reresolveSource(pool, S, sourceId, { log = () => {} } = {}) {
  const stats = {}
  const sup = await pool.query(
    `UPDATE ${S}.entity_links l SET status = 'superseded', superseded_at = now()
       FROM ${S}.source_entities se
      WHERE se.id = l.source_entity_id AND se.source_id = $1
        AND l.decided_by LIKE 'resolver:%' AND l.status IN ('accepted', 'candidate')`, [sourceId])
  stats.linksSuperseded = sup.rowCount
  const { rows: ents } = await pool.query(`SELECT id FROM ${S}.source_entities WHERE source_id = $1 ORDER BY id`, [sourceId])
  let done = 0
  for (const { id } of ents) {
    const d = await withTx(pool, (c) => resolveEntity(c, S, id))
    const k = `${d.action}${d.method ? `_${d.method}` : ''}`

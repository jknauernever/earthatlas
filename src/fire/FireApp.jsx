...
const FIRE_LAYERS = [
  {
    ...
    baseUrl: 'https://apps.fs.usda.gov/fsgisx01/rest/services/RDW_Wildfire/RMRS_WRC_WildfireHazardPotential/ImageServer',
    ...
    source: 'USDA Forest Service · RMRS Wildfire Hazard Potential',
  },
  {
    id: 'bp',
    label: 'Burn probability',
    baseUrl: 'https://imagery.geoplatform.gov/iipp/rest/services/Fire_Aviation/USFS_EDW_RMRS_ProbabilisticWildfireRiskBurnProbability/ImageServer',
    // Server-side colorizer — without it the service returns raw Int32 values.
    renderingRule: JSON.stringify({ rasterFunction: 'BurnProbability_32Bitsigned_RFt' }),
    defaultOpacity: 0.6,
    minZoom: 0,
    coverage: 'US incl. Alaska & Hawaii',
    legend: {
      // Matches the service's own YlOrRd ramp (decoded from its /legend
      // swatches): near-zero burn probability renders near-white and grades to
      // dark maroon at the highest class (~1-in-8 / year).
      kind: 'gradient',
      css: 'linear-gradient(to right, #ffffcc, #fed976, #fd8d3c, #fc4e2a, #e31a1c, #bd0026, #800026)',
      left: 'very low',
      right: 'high (annual)',
    },
    ...
    source: 'USDA Forest Service · RMRS Probabilistic Wildfire Risk — Burn Probability (2023)',
  },
  {
    id: 'ndvi',
...
// WHP is a classified product (native pixel value = class). Colors mirror the
// USFS WHP palette; classes 6/7 are the non-burnable / water sentinels.
const WHP_CLASSES = { 1: 'Very low', 2: 'Low', 3: 'Moderate', 4: 'High', 5: 'Very high', 6: 'Non-burnable', 7: 'Open water' }
const WHP_COLORS = { 1: '#38A800', 2: '#AAFF00', 3: '#FFFF00', 4: '#FFAA00', 5: '#FF0000', 6: '#b0b0b0', 7: '#419bdf' }
...
function interpretLayer(id, value) {
  ...
  if (id === 'whp') {
    ...
    const label = WHP_CLASSES[n]
    if (!label) return null
    const risk = n <= 2 ? 'low' : n === 3 ? 'moderate' : n <= 5 ? 'high' : null
    return { popupLabel: 'Wildfire hazard', value: label, swatch: WHP_COLORS[n] || null, risk, word: label }
  }
  if (id === 'bp') {
    const n = Number(value)
    if (!Number.isFinite(n)) return null
    if (n <= 0) return { popupLabel: 'Burn probability', value: 'Negligible', swatch: '#ffffcc', risk: 'low', oneIn: null }
    const prob = n / 1e6 // service stores annual probability × 1,000,000
...
function buildSummary(r) {
  ...
  // 4. A light interpretive nudge for the classic fire-prone combination.
  const fireRisk = (r.whp && r.whp.risk) || (r.bp && r.bp.risk)
  if (r.lulc && r.lulc.cover === 'Trees' && fireRisk === 'high') {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      FIRE_LAYERS.forEach((layer) => {
        // EE layers read their point value from the cloud function (?greenonly);
        // ArcGIS layers go through the identify proxy.
        const url = layer.kind === 'ee'
          ? `${TILES_API_BASE}?lat=${lat}&lng=${lng}&greenonly=1`
          : `${IDENTIFY_API}?base=${encodeURIComponent(layer.baseUrl)}&lat=${lat}&lng=${lng}`
        fetch(url)
          .then((r) => r.json())
          .then((d) => {
            const raw = layer.kind === 'ee' ? d.ndvi : d.value
            results[layer.id] = interpretLayer(layer.id, raw)
          })
